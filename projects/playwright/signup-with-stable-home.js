...
<body>
  ...
  const reportPath = path.join(__dirname, 'overdare-signup-stable-home-report.html');
  fs.writeFileSync(reportPath, html);
  return reportPath;
}

async function runSignupWithStableHome() {
  let driver;
  const signupReport = {
    testCase: '앱 안정화 후 회원가입 진행',
    success: false,
    successCount: 0,
    totalSteps: 0,
    steps: [],
  ...
  try {
    ...
    // 4단계: 홈 화면 도달 확인 및 안정화
    console.log('\n🏠 4단계: 홈 화면 도달 확인 및 안정화');
    
    let homeScreenReached = false;
    let attempts = 0;
    const maxAttempts = 10; // 더 많은 시도 횟수
    ...
    while (attempts < maxAttempts && !homeScreenReached) {
      ...
      if (foundHomeElements) {
        ...
        console.log('⏰ 앱 안정화 대기 중... (5초)');
        await driver.pause(5000);
        
        // 앱이 여전히 포그라운드에 있는지 확인
    ...
    console.error('❌ 앱 안정화 후 회원가입 자동화 중 오류 발생:', error.message);
...
// 스크립트 실행
runSignupWithStableHome().catch(console.error);
