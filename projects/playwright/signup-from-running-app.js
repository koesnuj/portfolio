...
<head>
    ...
    <title>Overdare 회원가입 자동화 리포트 (실행된 앱에서)</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎮 Overdare 회원가입 자동화 리포트 (실행된 앱에서)</h1>
            <p>이미 실행된 Overdare 앱에서 바로 회원가입 진행</p>
        </div>
...
async function signupFromRunningApp() {
  let driver;
  
  try {
    console.log('🎮 이미 실행된 Overdare 앱에서 회원가입 진행');
    console.log('='.repeat(60));
    
    // 1단계: 앱 연결 (자동 실행 방지)
    console.log('\n🔗 1단계: 실행된 앱에 연결');
    driver = await startOverdareApp(false); // 앱 자동 실행 방지
    await driver.pause(3000);
    
    const currentActivity = await driver.getCurrentActivity();
    console.log(`📍 현재 액티비티: ${currentActivity}`);
    const screenshot1 = await takeOverdareScreenshot(driver, 'step1-connected-to-app.png');
    recordStep('앱 연결', 'success', `Overdare 앱에 연결되었습니다. 액티비티: ${currentActivity}`, screenshot1);
    ...
    if (startButtonElement && await startButtonElement.isDisplayed()) {
      ...
      const screenshot4 = await takeOverdareScreenshot(driver, 'step4-start-clicked.png');
      recordStep('START 버튼 클릭', 'success', '게스트 회원가입이 시작되었습니다.', screenshot4);
    } else {
    ...
    if (sliderElement && await sliderElement.isDisplayed()) {
      ...
      await driver.pause(2000);
      const screenshot5 = await takeOverdareScreenshot(driver, 'step5-slider-moved.png');
      recordStep('슬라이더 조작', 'success', '나이 슬라이더가 조작되었습니다.', screenshot5);
    } else {
    ...
    if (okButtonElement && await okButtonElement.isDisplayed()) {
      ...
      await driver.pause(5000);
      const screenshot6 = await takeOverdareScreenshot(driver, 'step6-ok-clicked.png');
      recordStep('OK 버튼 클릭', 'success', 'OK 버튼이 클릭되어 다음 단계로 진행되었습니다.', screenshot6);
    } else {
    ...
    if (notificationScreenFound) {
      const screenshot7 = await takeOverdareScreenshot(driver, 'step7-notification-screen.png');
      recordStep('알림 설정 화면 도달', 'success', '회원가입이 완료되고 알림 설정 화면에 도달했습니다.', screenshot7);
    ...
    console.error('❌ 실행된 앱에서 회원가입 자동화 중 오류 발생:', error.message);
    ...
    const reportPath = path.join(__dirname, 'overdare-signup-from-running-app-report.html');
...
// 실행된 앱에서 회원가입 자동화 실행
signupFromRunningApp().catch(console.error);
