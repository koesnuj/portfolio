...
<body>
  ...
  const reportPath = path.join(__dirname, 'overdare-signup-prevent-background-report.html');
  fs.writeFileSync(reportPath, html);
  return reportPath;
}

async function runSignupWithPreventBackground() {
  let driver;
  const signupReport = {
    testCase: '백그라운드 이동 방지 후 회원가입 진행',
    success: false,
    successCount: 0,
    totalSteps: 0,
    steps: [],
  ...
  try {
    console.log('🎮 백그라운드 이동 방지 후 회원가입 진행');
    ...
    // 4단계: 홈 화면 도달 확인 및 백그라운드 이동 방지
    console.log('\n🏠 4단계: 홈 화면 도달 확인 및 백그라운드 이동 방지');
    
    let homeScreenReached = false;
    let attempts = 0;
    const maxAttempts = 15; // 더 많은 시도 횟수
    ...
    while (attempts < maxAttempts && !homeScreenReached) {
      ...
      if (currentActivity !== 'com.overdare.overdare.ui.MainActivity') {
        ...
        try {
          ...
          await driver.pause(2000); // 앱이 완전히 로드될 때까지 대기
        } catch (e) {
      ...
      if (foundHomeElements) {
        ...
        // 앱이 안정적으로 유지되도록 충분한 대기
        console.log('⏰ 앱 안정화 대기 중... (10초)');
        await driver.pause(10000);
        
        // 앱이 여전히 포그라운드에 있는지 재확인
        const finalActivity = await driver.getCurrentActivity();
        console.log(`📍 안정화 후 액티비티: ${finalActivity}`);
        
        if (finalActivity === 'com.overdare.overdare.ui.MainActivity') {
          console.log('✅ 앱이 안정적으로 유지되고 있습니다.');
        } else {
          console.log('⚠️ 앱이 다시 백그라운드로 이동했습니다.');
        }
        
        // 최종 스크린샷
        const screenshot4 = await takeOverdareScreenshot(driver, 'step4-home-screen-stable.png');
        recordStep('홈 화면 도달 및 안정화', 'success', `홈 화면에 도달하고 안정화되었습니다. 발견된 요소: "${homeElementText}"`, screenshot4);
    ...
    console.log('\n🎉 백그라운드 이동 방지 후 회원가입 자동화 완료!');
    console.log('='.repeat(60));

  } catch (error) {
    console.error('❌ 백그라운드 이동 방지 후 회원가입 자동화 중 오류 발생:', error.message);
...
// 스크립트 실행
runSignupWithPreventBackground().catch(console.error);
